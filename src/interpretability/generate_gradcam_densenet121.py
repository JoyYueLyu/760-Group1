...
MODEL_VARIANTS = {
    "standard": {
        ...
        "output_prefix": "densenet121",
        "display_name": "DenseNet121 + Standard CE",
    },
    "classweighted": {
        "model_path": MODEL_DIR / "densenet121_classweighted_best.pth",
        "output_prefix": "densenet121_classweighted",
        "display_name": "DenseNet121 + Class-weighted CE",
    },
    "ordinal": {
        "model_path": MODEL_DIR / "densenet121_ordinal_best.pth",
        "output_prefix": "densenet121_ordinal",
        "display_name": "DenseNet121 + Ordinal-aware Loss",
    },
}
...
def select_samples(test_df, samples_per_class):
    ...
    if not selected_rows:
        raise ValueError("No test samples were selected for Grad-CAM.")

    selected_df = pd.concat(selected_rows, ignore_index=True)

    if selected_df.empty:
        raise ValueError("Selected Grad-CAM dataframe is empty.")
...
def main():
    ...
    parser.add_argument(
        ...
        help="Which DenseNet121 model variant to visualize."
    )
    ...
    prediction_path = RESULT_DIR / f"{output_prefix}_gradcam_predictions.csv"
