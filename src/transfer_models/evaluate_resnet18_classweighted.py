...
def save_confusion_matrix(cm, class_names, output_path):
    ...
    plt.title("ResNet18 Class-weighted Loss Confusion Matrix")
    plt.colorbar()
...
def main():
    ...
    print("=" * 50)
    print("Evaluating ResNet18 Class-weighted Loss Model on Test Set")
    print("=" * 50)
    ...
    model_path = MODEL_DIR / "resnet18_classweighted_best.pth"

    if not model_path.exists():
        raise FileNotFoundError(
            f"Cannot find model file: {model_path}. "
            "Please run train_resnet18_weighted.py first."
        )
    ...
    overall_path = RESULT_DIR / "resnet18_classweighted_test_metrics.csv"
    overall_df.to_csv(overall_path, index=False)

    per_class_path = RESULT_DIR / "resnet18_classweighted_per_class_metrics.csv"
    ...
    predictions_path = RESULT_DIR / "resnet18_classweighted_test_predictions.csv"
    ...
    cm_csv_path = RESULT_DIR / "resnet18_classweighted_confusion_matrix.csv"
    cm_df.to_csv(cm_csv_path)

    cm_fig_path = FIGURE_DIR / "resnet18_classweighted_confusion_matrix.png"
