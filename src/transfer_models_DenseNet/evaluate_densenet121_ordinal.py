...
MODEL_NAME = "densenet121_ordinal"
...
def save_confusion_matrix(cm, class_names, output_path):
    ...
    plt.title("DenseNet121 Ordinal-aware Loss Confusion Matrix")
    plt.colorbar()
...
def main():
    ...
    print("=" * 50)
    print("Evaluating DenseNet121 Ordinal-aware Loss Model on Test Set")
    print("=" * 50)
    print(f"Device: {device}")

    _, _, test_loader = get_dataloaders()

    model_path = MODEL_DIR / "densenet121_ordinal_best.pth"

    if not model_path.exists():
        raise FileNotFoundError(
            f"Cannot find model file: {model_path}. "
            "Please run train_densenet121_ordinal.py first."
        )
    ...
    overall_path = RESULT_DIR / "densenet121_ordinal_test_metrics.csv"
    overall_df.to_csv(overall_path, index=False)

    per_class_path = RESULT_DIR / "densenet121_ordinal_per_class_metrics.csv"
    ...
    predictions_path = RESULT_DIR / "densenet121_ordinal_test_predictions.csv"
    ...
    cm_csv_path = RESULT_DIR / "densenet121_ordinal_confusion_matrix.csv"
    cm_df.to_csv(cm_csv_path)

    cm_fig_path = FIGURE_DIR / "densenet121_ordinal_confusion_matrix.png"
